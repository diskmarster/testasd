...
import { UserInfo, UserNoHashWithCompany } from '@/data/user.types'
...
export type UserActivationLink =
	`${typeof ACTIVATION_LINK_BASEURL}/invitering/${UserLinkID}`
...
export const userService = {
	...
	updatePassword: async function (
		...
		const updatedAuth = await user.updateAuthProvider(
			userID,
			'pw',
			hashedPassword,
		)
	...
	createInvitedUser: async function (
		userLinkData: UserLink,
		newUserData: NewUser,
	): Promise<UserNoHash | undefined> {
	...
	createUserLink: async function (
		userLinkData: Omit<NewUserLink, 'id'>,
	): Promise<UserActivationLink | undefined> {
	...
	getInviteLinkByID: async function (
		linkID: UserLinkID,
	...
	getByIDs: async function (userIDs: UserID[]): Promise<UserNoHash[]> {
