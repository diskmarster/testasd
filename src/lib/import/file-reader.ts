...
export function readAndValidateFileData<
...
>(
	...
	return new Promise((resolve, reject) => {
		...
		const expectedHeaders =
			config.expectedHeaders ?? defaultConfig.expectedHeaders
		const transformHeaders =
			config.transformHeaders ?? defaultConfig.transformHeaders
		...
		reader.onload = () => {
			try {
				...
				const missingHeaders = expectedHeaders.filter(
					h => !transformedHeaders.includes(h),
				)
						...
						errors: new z.ZodError([
							{
								...
								message: t('products.header-missing', {
									...
									header: missingHeaders
										.map(h => t('products.header-name', { context: h }))
										.join(', '),
								}),
							},
						]),
