...
export async function LocationMoveServerWrapper({ user }: LocationMoveWrapper.Props) {
	...
	const currentLocationInventories = await inventoryService.getInventory(user.customerID, currentLocation!)
	let customerSettings = await customerService.getSettings(user.customerID)
	const useBatch = await productService.getBatchProducts(user.customerID).then(p => p.length > 0)
