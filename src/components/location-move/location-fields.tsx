			...
			Array.from(
				inventories
					.reduce((acc, cur) => {
						...
						return acc
					}, new Map<string, FormattedInventory['product']>())
					.values(),
			),
		[],
	)
				...
				{fields.length > 0 ? (
					fields.map((f, fi) => {
						...
						return (
							<div
								key={`${fi}-${f.sku}`}
								className='grid grid-cols-[1fr_1fr_1fr_1fr_1fr_auto] items-end gap-2'>
								<div className='grid gap-1.5'>
									<Label className={cn(fi != 0 && 'hidden')}>
										{t('sku-label')}
									</Label>
								...
								{usePlacements && (
									<div className='grid gap-1.5'>
										<Label className={cn(fi != 0 && 'hidden')}>
											{t('from-placement-label')}
										</Label>
										<Select
											value={form
												.getValues(`fields.${fi}.fromPlacementID`)
												?.toString(10)}
											onValueChange={value => {
												form.setValue(
													`fields.${fi}.fromPlacementID`,
													Number(value),
											...
											<SelectTrigger
												className={cn(
													error &&
														error.type == 'from-placement' &&
														'border-destructive',
												)}>
												<SelectValue
													placeholder={t('from-placement-placeholder')}
												/>
											</SelectTrigger>
											<SelectContent>
												{productPlacements
													.get(form.getValues(`fields.${fi}.sku`))
													?.map((p, pi) => (
														<SelectItem
															key={`${pi}-${p.name}`}
															value={p.id.toString(10)}>
															{p.name} {p.isBarred && 'spærret'}
														</SelectItem>
													))}
											</SelectContent>
										</Select>
								...
								{useBatches && (
									<div className='grid gap-1.5'>
										...
										<Select
											disabled={
												form.getValues(`fields.${fi}.fromPlacementID`) === -1
											}
											value={form
												.getValues(`fields.${fi}.fromBatchID`)
												?.toString(10)}
											onValueChange={value => {
												form.setValue(
													`fields.${fi}.fromBatchID`,
													Number(value),
													{ shouldDirty: true, shouldValidate: true },
												)
											}}>
											<SelectTrigger
												className={cn(
													error &&
														error.type == 'from-batch' &&
														'border-destructive',
												)}>
												<SelectValue
													placeholder={t('from-batch-placeholder')}
												/>
											</SelectTrigger>
											<SelectContent>
												{productBatches
													.get(
														`${form.getValues(`fields.${fi}.sku`)}-${form.getValues(`fields.${fi}.fromPlacementID`)}`,
													)
													?.map((b, pi) => (
														<SelectItem
															key={`${pi}-${b.id}`}
															value={b.id.toString(10)}>
															{b.batch} {b.isBarred && 'spærret'}
														</SelectItem>
													))}
											</SelectContent>
										</Select>
								...
								<div className='grid gap-1.5'>
									...
									<Select
										...
										onValueChange={value => {
											form.setValue(`fields.${fi}.toLocationID`, value, {
												shouldDirty: true,
												shouldValidate: true,
											})
										}}>
										...
										<SelectContent>
											{locations
												.filter(loc => loc.id != form.getValues('fromLocation'))
												.map((l, li) => (
													<SelectItem key={`${li}-${l.name}`} value={l.id}>
														{l.name} {l.isBarred && 'spærret'}
													</SelectItem>
												))}
										</SelectContent>
									</Select>
								...
								<div className='grid gap-1.5'>
									...
									<NumberInput
										...
										decimalScale={2}
										className={cn(
											error && error.type == 'quantity' && 'border-destructive',
										)}
										defaultValue={f.quantity}
										onValueChange={value => {
											form.setValue(`fields.${fi}.quantity`, value ?? 0, {
												shouldDirty: true,
												shouldValidate: true,
											})
										}}
					...
					<Command className='border-t rounded-t-none'>
						...
						<CommandList>
							...
							<CommandGroup>
								{products
									.filter(
										p => !form.getValues('fields').some(f => f.sku == p.sku),
									)
									.map((p, i) => (
										<CommandItem
											...
											value={p.id.toString(10)}
											onSelect={() => {
												const prevToLocationId =
													form.getValues(
														`fields.${fields.length - 1}.toLocationID`,
													) ?? ''
