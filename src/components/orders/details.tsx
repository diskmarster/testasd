...
} from '@/app/[lng]/(site)/genbestil/[id]/actions'
...
export function Details({ order, customer, user }: Props) {
	...
	const lines = order.lines.sort((lA, lB) =>
		lB.supplierName.localeCompare(lA.supplierName),
	)
	...
	function downloadOrder() {
		const rows: ExcelRow[] = order.lines.map(l => ({
		...
		const singleSupplier =
			order.supplierCount == 1 && order.lines[0].supplierName != '-'
				? {
				...
				: undefined
		const workbook = genReorderExcelWorkbook(
			order.id,
			order.inserted,
			user,
			customer,
		...
		XLSX.writeFile(
			workbook,
			`nemlager_genbestilling_${formatDate(new Date())}.xlsx`,
		)
	}
...
function ViewItem({
	...
	const lng = useLanguage()
	return (
		<div className={cn('text-sm px-3 py-2', gridClasses)}>
			<p>{line.sku}</p>
			...
			<p className='text-right tabular-nums'>
				{numberToCurrency(line.costPrice, lng)}
			</p>
			<p className='text-right tabular-nums'>
				{formatNumber(line.quantity, lng)}
			</p>
			<p>{line.unitName}</p>
			<p className='text-right tabular-nums'>
				{numberToCurrency(line.sum, lng)}
			</p>
		</div>
	)
}
...
function SendEmailModal({
	...
	function onSubmit() {
		const data = {
			orderID: order.id,
			emails: input ? [...selected, input] : selected,
		}
							...
							<div
								...
								<div>
									<p className='text-xs font-medium'>
										{order.lines[0].supplierName}
									</p>
									<p className='text-xs text-muted-foreground'>
										{order.lines[0].supplierEmail}
									</p>
								</div>
								{selected.includes(order.lines[0].supplierEmail) && (
