...
export function CreateProductsForm({
	...
	const { handleSubmit, register, formState, setValue, reset, control } =
		...
		<Credenza open={show} onOpenChange={onOpenChange}>
			<CredenzaTrigger asChild>
				<Button
					size='icon'
					variant='outline'
					tooltip={t('tooltips.create-product')}>
							...
							<ScrollArea>
								<div className='grid gap-2'>
									<p className='text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70'>
										{t('default-placement', { count: locations.length })}
									</p>
									<div className='grid gap-4 p-2 border rounded-sm'>
											...
											<>
												{locations.length > 1 ? (
													...
													<SelectDefaultPlacement
														...
														placements={locationPlacementMap.get(
															locations[0].id,
														)}
														field={fields[0]}
														setValue={setValue}
										...
										{locations.length > fields.length && (
											<Button
												...
												onClick={() =>
													append({ locationID: '', placementID: -1 })
												}>
...
function SelectDefaultPlacement({
	...
	const locationID = useMemo(() => location.id, [location])
	...
	return (
		<div className='grid grid-cols-[398px_36px] gap-2'>
			...
			<Button
				className='self-end hover:text-destructive-foreground hover:bg-destructive'
				variant={'outline'}
				size={'icon'}
				onClick={() => remove(0)}>
			...
			{fields.map((field, index) => (
				<SelectDefaultPlacementsItem
					...
					locations={locations.filter(
						loc =>
							field.locationID == loc.id ||
							!fields.some(f => f.locationID == loc.id),
					)}
					locationPlacementMap={locationPlacementMap}
					field={field}
					index={index}
					setValue={(key: 'locationID' | 'placementID', val: string | number) =>
						setValue(`defaults.${index}.${key}`, val, {
							shouldValidate: true,
							shouldDirty: true,
						})
					}
...
interface SelectDefaultPlacementsItemProps<
	TField extends FieldArrayWithId<
		{
...
> {
	...
	index: number
	setValue: <
		TKey extends Exclude<keyof TField, 'id'>,
		TValue extends TField[TKey],
	>(
		key: TKey,
		value: TValue,
	) => void
	...
	return (
		<div className='grid grid-cols-[195px_195px_36px] gap-2'>
			...
			<div className='grid gap-2'>
				...
				<Select
					value={
						selectedPlacementID != -1 ? selectedPlacementID.toString() : ''
					}
					onValueChange={id => updatePlacementID(parseInt(id))}>
					<SelectTrigger
						className='w-[195px]'
						disabled={locationPlacements == undefined}>
						<SelectValue placeholder={t('select-placement')} />
					</SelectTrigger>
					<SelectContent>
