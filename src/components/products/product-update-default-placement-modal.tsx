...
export function UpdateDefaultPlacementModal({ productID }: Props) {
	const lng = useLanguage()
	const { t } = useTranslation(lng, 'produkter', {
		keyPrefix: 'details-page.inventory.default-placement-dialog',
	})
		...
		if (newPlacement != undefined) {
			startTransition(async () => {
				...
				if (res && res.serverError) {
					...
					emitCustomEvent(
						`DefaultPlacementUpdated-${newPlacement.locationID}`,
						{ placementID: newPlacement.id },
					)
					setOpen(false)
				}
			...
			<DialogContentV2>
				...
				<section className='px-4 tracking-tight tabular-nums space-y-3'>
					<p>
						...
						. {t('update-will-move-inventories')}{' '}
						<span className='tracking-tighter font-semibold'>
							{newPlacement?.name}
						</span>
						.
