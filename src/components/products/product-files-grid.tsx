		...
		<>
			<div className='lg:w-1/2 flex-1 border rounded-md p-4 flex flex-col gap-4'>
				<div className='flex items-center gap-1.5'>
					...
					{pending && <Icons.spinner className='size-3 animate-spin' />}
				</div>
				<div className='space-y-2'>
					<p className='text-sm text-muted-foreground'>
						{t('details-page.files.documents')}
					</p>
					<div className='flex flex-col'>
						{!pdf && (
							<p className='text-muted-foreground'>
								{t('details-page.files.no-documents')}
							</p>
						)}
						{pdf &&
							pdf.map((p, i) => (
				...
				<div className='space-y-2'>
					<p className='text-sm text-muted-foreground'>
						{t('details-page.files.images')}
					</p>
					<div className='flex gap-2 flex-wrap'>
						{!image && (
							<p className='text-muted-foreground'>
								{t('details-page.files.no-images')}
							</p>
						)}
						{image &&
							image.map((f, i) => (
...
function PDFFile({ file, user }: { file: Attachment; user: User }) {
	const lng = useLanguage()
	const { t } = useTranslation(lng, 'produkter')
	return (
		<div>
			<div className='flex items-center justify-between'>
				<div className='flex items-center gap-1.5 group/pdf cursor-pointer'>
					<div className='text-[10px] px-1 py-0.5 rounded-sm bg-red-500 font-semibold text-white'>
						PDF
					</div>
					<Link
						href={file.url}
						target='_target'
						className='group-hover/pdf:underline text-sm'>
						{file.name}
					</Link>
				</div>
				{hasPermissionByRank(user.role, 'bruger') && (
					<DropdownMenu>
						...
						<DropdownMenuContent align='end'>
							<DropdownMenuItem>
								{t('details-page.files.document-show')}
							</DropdownMenuItem>
							<DropdownMenuSeparator />
							<DropdownMenuItem
								className='text-destructive'
								onClick={() => {
									emitCustomEvent('DeleteFileByID', {
										id: file.id,
										refID: file.refID,
									})
								}}>
								{t('details-page.files.document-delete')}
							</DropdownMenuItem>
						</DropdownMenuContent>
					</DropdownMenu>
				)}
...
function ImageFile({
	...
	return (
		<div className='border rounded-md flex items-center justify-center overflow-hidden group/file-image cursor-pointer max-w-20 aspect-square relative'>
			<div
...
function FileDropZone({
		...
		async (files: File[], rejectedFiles: FileRejection[]) => {
			if (rejectedFiles.length > 0) {
				for (const file of rejectedFiles) {
					toast.error(t(`common:${siteConfig.errorTitle}`), {
						description: t('details-page.files.file-rejected', {
							context: file.errors[0].code,
							name: file.file.name,
						}),
					})
				}
			...
			for (const file of files) {
				...
				if (!isValidated.success) {
					toast.error(t(`common:${siteConfig.errorTitle}`), {
						description: isValidated.error,
					})
					continue
				}

				const uploadPromise = uploadFileAction({
					key: isValidated.key,
					type: file.type,
					body: base64,
				})
				...
				if (
					...
					const description =
						uploadResponse.serverError ?? t('details-page.files.upload-error')
					toast.error(t(`common:${siteConfig.errorTitle}`), { description })
					const deleteAttach = await deleteAttachmentAction({
						id: attachmentResponse.data.attachment.id,
					})

					if (deleteAttach && deleteAttach.serverError) {
						toast.error(t(`common:${siteConfig.errorTitle}`), {
							description: deleteAttach.serverError!,
						})
					}
					continue
				}
	...
	return (
		<div
			className={cn(
				'mt-auto border-dashed border py-4 rounded-md grid place-items-center hover:border-primary transition-colors cursor-pointer group/dropzone',
			...
			<div className='flex items-center gap-2'>
				{filesLoading > 0 && filesLoading > filesDone ? (
					<>
						...
						<p className='text-sm'>
							{t('details-page.files.currently-uploading', {
								current: filesDone,
								max: filesLoading,
							})}
						</p>
					...
					<>
						...
						<p className='text-sm'>
							{isDragActive
								? t('details-page.files.drag-active')
								: t('details-page.files.drag-inactive')}
						</p>
					</>
				)}
			</div>
...
function DeleteFileModal() {
	...
	function deleteFile() {
		...
		startTransition(async () => {
			...
			onOpenChange(false)
			toast.success('Fil blev slettet', {
				description: t('details-page.files.delete-success'),
			})
	...
	return (
		<AlertDialog open={open} onOpenChange={onOpenChange}>
			<AlertDialogContent className='max-w-sm'>
				<AlertDialogHeader>
					<AlertDialogTitle>
						{t('details-page.files.delete-title')}
					</AlertDialogTitle>
					<AlertDialogDescription>
						{t('details-page.files.delete-description')}
					</AlertDialogDescription>
				</AlertDialogHeader>
				<div className='flex items-center gap-2'>
					<Button
