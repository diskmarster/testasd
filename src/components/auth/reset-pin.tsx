...
import { resetPinAction } from '@/app/[lng]/(auth)/glemt-password/actions'
import { resetPinValidation } from '@/app/[lng]/(auth)/glemt-password/validation'
...
export function ResetPinCard({ link }: { link?: ResetPassword }) {
	...
	const [passwordResat, setPasswordResat] = useState<boolean>(
		successParam == null ? false : Boolean(successParam),
	)
	...
	if (
		!link ||
		!resetPasswordTypes.includes(link.passwordType) ||
		isBefore(link.expiresAt, Date.now())
	) {
		return (
			<div className='mx-auto max-w-lg space-y-4 text-center'>
	...
	return (
		<Card className='relative w-full max-w-sm mx-auto'>
			<CardHeader>
				...
				<CardDescription>{t('reset-pin-card.new-pin')}</CardDescription>
			</CardHeader>
			<CardContent>
		...
		<form
			...
			<div className='grid gap-2'>
				<Label htmlFor='confirmPassword'>
					{t('reset-pin-card.confirm-new-pin')}
				</Label>
