...
} from '@/app/[lng]/(site)/administration/organisation/actions'
...
export function ModalEditUser({}: Props) {
	...
	const rolesOptions = userRoles
		.filter(role => role != 'system_administrator' && role.toLowerCase())
		.map(role => ({
						...
						{!user ? (
							...
							<form
										...
										<div className='flex items-center justify-between h-[14px]'>
											...
											{locations && (
												<span
													...
													{' af '}
													{locations.length}{' '}
													{t('modal-edit-user.locations-chosen')}
												</span>
											)}
										</div>
								...
								<Button
									...
									type='submit'
									disabled={
										pending || !formState.isDirty || !formState.isValid
									}>
