	...
	function onSubmit(values: z.infer<typeof validationSchema>) {
		...
		startTransition(async () => {
			...
			for (let i = 0; i < chunkedArray.length; i++) {
				...
				const errorMsg = t('import-inventory-modal.error-from', {
					from: start + 1,
			...
			<DialogContent className='max-w-xl'>
				...
				<div className='flex flex-col gap-4'>
					...
					<div
						...
						<div className='text-muted-foreground text-sm grid place-items-center'>
							{isReading ? (
								...
								<p>
									{t('import-inventory-modal.loaded-and-ready', {
										amount: rows.length,
									})}
								</p>
							) : (
						...
						<div className='flex flex-col gap-1 text-destructive text-sm'>
							...
							{errors.issues.slice(0, 5).map((issue, i) => {
								...
								return (
									<div key={i}>
										<p>
											{t('import-inventory-modal.error-message', {
												rowNum: rowNumber,
												rowKey: rowKey,
												rowMsg: rowMsg,
											})}
					...
					<Button
						...
						{pending && rows.length > 0
							? t('import-inventory-modal.upload-progress', {
									uploaded: uploadedAmount,
									max: rows.length,
								})
