...
import { importInventoryDataValidation } from "@/app/[lng]/(site)/sys/kunder/validation"
...
import { fetchLocationsForCustomerActions, importInventoryAction } from "@/app/[lng]/(site)/sys/kunder/actions"
import { CustomerID } from "@/lib/database/schema/customer"
import { LocationWithCounts } from "@/data/location.types"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "../ui/select"

export function ModalImportClientInventory() {
  ...
  useCustomEventListener("ImportClientInventoryByID", (data: any) => {
  ...
  function onSubmit(values: z.infer<typeof validationSchema>) {
    ...
    startTransition(async () => {
      ...
      for (let i = 0; i < chunkedArray.length; i++) {
        ...
        const errorMsg = t('import-inventory-modal.error-from', { from: start + 1, to: start + chunk.length })
        const res = await importInventoryAction({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>{t('import-inventory-modal.title')}</DialogTitle>
          <DialogDescription>
            {t('import-inventory-modal.description')}
          </DialogDescription>
        </DialogHeader>
        <div className='flex flex-col gap-4'>
          <div className="grid gap-2">
            <Label>{t('import-inventory-modal.location-label')}</Label>
          ...
          <div className='grid gap-2'>
            <Label>{t('import-inventory-modal.before-upload')}</Label>
            <p className='text-muted-foreground text-sm'>
              {t('import-inventory-modal.import-criteria')}
            </p>
          </div>
          <div className='border rounded-md p-4 flex items-center justify-between gap-4'>
            <div className='flex flex-col gap-1'>
              <div className='flex items-center gap-2'>
                ...
                <p className='text-sm'>
                  {t('import-inventory-modal.file-example')}
                </p>
              </div>
              <p className='text-xs text-muted-foreground'>
                {t('import-inventory-modal.file-import-example')}
              </p>
            </div>
            <a
              ...
              download>
              {t('import-inventory-modal.download')}
            </a>
          </div>
          <div
            {...getRootProps()}
            ...
            <div className='text-muted-foreground text-sm grid place-items-center'>
              {isReading ? (
                <div className='flex gap-2 items-center'>
                  <p>{t('import-inventory-modal.loading-the-file')}</p>
                  <Icons.spinner className='animate-spin size-3' />
                </div>
              ) : rows.length > 0 ? (
                <p>
                  {t('import-inventory-modal.loaded-and-ready', { amount: rows.length })}
                </p>
              ) : (
                <p>
                  {isDragActive
                    ? t('import-inventory-modal.drag-and-drop2')
                    : t('import-inventory-modal.drag-and-drop')}
                </p>
              )}
            </div>
          </div>
          {errors && (
            <div className='flex flex-col gap-1 text-destructive text-sm'>
              ...
              {errors.issues.slice(0, 5).map((issue, i) => {
                ...
                return (
                  <div key={i}>
                    <p>{t('import-inventory-modal.error-message', { rowNum: rowNumber, rowKey: rowKey, rowMsg: rowMsg })}</p>
                  </div>
                )
              })}
              {errors.issues.length > 5 && (
                <p className='text-foreground'>
                  {errors.issues.length - 5}{' '}
                  {t('import-inventory-modal.more-errors')}
                </p>
              )}
            </div>
          )}
          ...
          {isDone && responseErrors.length == 0 && (
            <Alert variant='default' className='border-success'>
              ...
              <AlertTitle className='text-success font-semibold'>
                {t('import-inventory-modal.completed-title')}
              </AlertTitle>
          ...
          <Button
            ...
            {(pending && rows.length > 0)
              ? t('import-inventory-modal.upload-progress', { uploaded: uploadedAmount, max: rows.length })
              : t('import-inventory-modal.upload-button')}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
