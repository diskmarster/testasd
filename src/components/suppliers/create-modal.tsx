...
export function CreateSupplierModal({ }: Props) {
	...
	const { handleSubmit, reset, register, setValue, watch, formState } = useForm<z.infer<typeof createSupplierValidation>>({
	...
	return (
		<DialogV2 open={open} onOpenChange={onOpenChange}>
			...
			<DialogContentV2 className="max-w-md">
					...
					<div className="flex items-center gap-2">
						<div className="grid gap-1.5">
							...
							<Select
								...
								onValueChange={(val: SupplierContry) => setValue('country', val, { shouldValidate: true })}
							>
						...
						<div className="grid gap-1.5 w-full">
							...
							<Input {...register('name')} name="name" id="name" autoFocus type="text" />
						</div>
					</div>
					<div className="grid gap-1.5 w-full">
						<Label htmlFor="idOfClient">
							{t("create.idOfClient")}
						</Label>
						<Input {...register('idOfClient')} name="idOfClient" id="idOfClient" type="text" />
					</div>
					<div className="grid gap-1.5 w-full">
						<Label htmlFor="contactPerson">
							{t("create.contact-person")}
						</Label>
						<Input {...register('contactPerson')} name="contactPerson" id="contactPerson" type="text" />
					</div>
					<div className="flex items-center gap-2">
						<div className="grid gap-1.5 w-full">
							<Label htmlFor="phone">
							...
							<Input {...register('phone')} name="phone" id="phone" type="tel" />
						</div>
						<div className="grid gap-1.5 w-full">
							<Label htmlFor="email">
								{t("create.email")}
				...
				<DialogFooterV2>
					<Button onClick={() => onOpenChange(false)} size='sm' variant='outline'>{t("create.btn-close")}</Button>
