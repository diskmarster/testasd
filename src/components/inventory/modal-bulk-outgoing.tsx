...
import { BulkError, bulkOutgoingAction, fetchPlacementInventories } from "@/app/[lng]/(site)/oversigt/actions"
...
export function ModalBulkOutgoing({ placements, useBatch, useReference }: Props) {
	...
	async function fetchInventories(placementID: PlacementID) {
		...
		if (response.serverError || response.validationErrors) {
			toast.error(t(`common:${siteConfig.errorTitle}`), {
				description: response.serverError ?? response.validationErrors?._errors?.flatMap(v => v)
			})
			setIsFetching(false)
			return
		}
		if (response.data) {
			const nonnegativeInventories = response.data.inventories.filter(inv => inv.quantity > 0)
	...
	function submit() {
		startTransition(async () => {
			...
			if (response.data && !response.data.ok) {
				...
				const failedInventories = inventories.filter(inv => data.errors!.some(e => e.productID == inv.product.id))
		...
		<DialogV2 open={open} onOpenChange={onOpenChange}>
			...
			<DialogContentV2 className="max-w-2xl">
				...
				<div className="space-y-4 px-3">
					<DialogDescriptionV2 className="text-sm text-muted-foreground">{t("desc")}</DialogDescriptionV2>
						...
						<div className="flex items-center gap-2">
							<div className="grid gap-1.5 w-full max-w-[50%]">
								...
								<Select onValueChange={(val) => {
									...
									<SelectContent>
										{placements.map(p => (
											<SelectItem key={p.id} value={p.id.toString(10)}>{p.name}</SelectItem>
										))}
									</SelectContent>
								</Select>
							</div>
							{useReference && (
								<div className="grid gap-1.5 w-full">
									...
									<Input value={reference} onChange={e => setReference(e.target.value)} placeholder={t("reference-placeholder")} />
								</div>
							)}
						</div>
						<div className="space-y-2">
							...
							{inventories.length > 0 ? (
								<div className="space-y-1 relative border rounded-md shadow-sm bg-background">
									<div className={cn("grid px-3 gap-2 text-xs text-muted-foreground sticky top-0 bg-background py-2 font-medium border-b", useBatch ? "grid-cols-[1fr_1.5fr_1fr_1fr_0.5fr]" : "grid-cols-[1fr_1.5fr_1fr_0.5fr]")}>
										<p>{t("sku-label")}</p>
										<p>{t("text1-label")}</p>
									...
									<ScrollArea maxHeight="max-h-96">
										<div className="divide-y">
											{inventories.map(inv => {
												const err = bulkErrors.find(err => err.productID == inv.product.id)
												return (
													<div
														className={cn("grid px-3 gap-2 text-sm py-1.5", useBatch ? "grid-cols-[1fr_1.5fr_1fr_1fr_0.5fr]" : "grid-cols-[1fr_1.5fr_1fr_0.5fr]", err && "text-destructive")}
														key={`${inv.customerID}-${inv.product.id}-${inv.placement.id}-${inv.batch.id}`}>
														<p>{inv.product.sku}</p>
														<p className="truncate">{inv.product.text1.substring(0, 24)}</p>
								...
								isFetching ? (
									...
									<p className="text-sm">{selectedPlacement
										? t("no-inventories")
				...
				<DialogFooterV2>
					<Button disabled={pending} variant="outline" onClick={() => onOpenChange(false)}>{t("btn-close")}</Button>
					<Button disabled={pending} onClick={submit} className="flex items-center gap-2">
						{pending && (
							<Icons.spinner className="animate-spin size-4" />
						)}
						{t("btn-confirm")}
