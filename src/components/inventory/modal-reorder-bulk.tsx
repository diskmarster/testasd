...
export function ModalBulkReorder({ reorders, productsWithNoReorders }: Props) {
	...
	const { handleSubmit, reset, register, setValue, watch, formState, control } =
	...
	const selectableProducts = useMemo(() => {
		return productsWithNoReorders.filter(
			p =>
				selectableReorders.some(r => r.productID != p.id) ||
	...
	useCustomEventListener(
		'BulkReorder',
		({ reorders: newReorders }: { reorders: FormattedReorder[] }) => {
			reset({
	...
	function appendSupplier(sup: string) {
		const reordersWithSupplier = reorders.filter(
			r =>
				r.product.supplierName == sup && !selectedProductIDs.has(r.productID),
		)
		reordersWithSupplier.forEach(r =>
			append({
				sku: r.product.sku,
				ordered: r.orderAmount,
				alreadyOrdered: r.ordered,
	...
	const totalPrice = formValues.items.reduce(
		(acc, cur) => acc + cur.ordered * cur.costPrice,
		0,
	)
							...
							<div className='flex items-center gap-2'>
								<Popover
									...
									<PopoverContent className='p-0 min-w-96 max-w-96'>
										<Command>
											<CommandInput
												placeholder={t('bulk.product-search')}
												className='h-9'
											/>
											<CommandList>
												<CommandEmpty>
													{t('bulk.product-no-items')}
												</CommandEmpty>
												<CommandGroup>
																...
																<div className='text-xs flex items-center gap-1 text-muted-foreground'>
																	...
																	{p.supplierName && (
																		<>
																			<span>-</span>
																			<span>
																				{t('bulk.supplier')}: {p.supplierName}
																			</span>
																		</>
																	)}
																</div>
								...
								{selectableSuppliers.length > 0 && (
									<Popover
										...
										<PopoverContent className='p-0 min-w-52 max-w-52'>
											<Command>
												<CommandInput
													placeholder={t('bulk.suppliers-search')}
													className='h-9'
												/>
												<CommandList>
													<CommandEmpty>
														{t('bulk.suppliers-no-items')}
													</CommandEmpty>
													<CommandGroup>
														{Array.from(
															new Set(
																selectableSuppliers.map(
																	s => s.product.supplierName,
																),
															).values(),
														)
															.slice(0, 50)
...
function ReorderField({
	...
	const max =
		formValues.items[index].maxOrderAmount > 0
			? formValues.items[index].maxOrderAmount
			: undefined

	return (
		<div className='flex items-end gap-2'>
			<div className='grid gap-1.5 w-[60%]'>
				<Label className={cn('', index !== 0 && 'hidden')}>
					{t('bulk.product')}
				</Label>
					...
					<PopoverTrigger asChild>
						<Button
							...
							<p className='max-w-[80%] truncate'>
								{search ? field.text1 : t('bulk.product-placeholder')}
							</p>
								...
								<CommandGroup>
									{selectableReorders.slice(0, 50).map((p, i) => (
										<CommandItem
											...
											onSelect={currentValue => {
												setValue(`items.${index}.productID`, p.productID, {
													shouldValidate: true,
												})
												setValue(`items.${index}.sku`, p.product.sku, {
													shouldValidate: true,
												})
												setValue(`items.${index}.text1`, p.product.text1, {
													shouldValidate: true,
												})
												setValue(`items.${index}.alreadyOrdered`, p.ordered, {
													shouldValidate: true,
												})
												setValue(`items.${index}.quantity`, p.quantity, {
													shouldValidate: true,
												})
												setValue(`items.${index}.disposable`, p.disposible, {
													shouldValidate: true,
												})
												setValue(
												...
												setValue(`items.${index}.ordered`, p.orderAmount, {
													shouldValidate: true,
												})
												setValue(
													`items.${index}.maxOrderAmount`,
													p.maxOrderAmount,
													{ shouldValidate: true },
												)
												setValue(`items.${index}.isRequested`, p.isRequested, {
													shouldValidate: true,
												})
												setValue(`items.${index}.groupName`, p.product.group, {
													shouldValidate: true,
												})
												setValue(`items.${index}.unitName`, p.product.unit, {
													shouldValidate: true,
												})
												setValue(
			...
			<div className='grid gap-1.5 w-[30%]'>
				...
				<Input value={formValues.items[index].supplierName ?? '-'} disabled />
			</div>
			<div className='grid gap-1.5 w-[15%]'>
				...
				<Input
					...
					value={field.isRequested ? '-' : formatNumber(field.minimum, lng)}
					disabled
				/>
			</div>
			<div className='grid gap-1.5 w-[15%]'>
			...
			<div className='grid gap-1.5 w-[15%]'>
				...
				<Input
					...
					tooltip={{
						...
						content:
							formState.errors.items?.[index]?.ordered?.message ??
							'Forkert værdi',
					}}
					...
					className={cn(
						...
						((formState.errors.items &&
							formState.errors.items[index]?.ordered) ||
							formValues.items[index].ordered == 0) &&
			...
			<div className='grid gap-1.5 w-[20%]'>
				...
				<Input
					...
					value={numberToCurrency(
						formValues.items[index].ordered * field.costPrice,
					)}
					disabled
				/>
			</div>
