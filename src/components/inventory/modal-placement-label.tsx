...
export function ModalPlacementLabelTrigger({
	size = 'icon',
	variant = 'ghost',
	...
	function onclick() {
		emitCustomEvent('ModalPlacementLabel', labelData)
	}
...
export function ModalPlacementLabel() {
	...
	useCustomEventListener('ModalPlacementLabel', (data: typeof rows) => {
		setRows(data)
		setOpen(true)
	})
	...
	function getPdf() {
		startTransition(async () => {
			const res = await preparePlacementLabelsPDFAction({
				size: [width, height],
				copies,
				placements: rows,
			})
