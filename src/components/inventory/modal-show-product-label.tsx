			...
			<CredenzaContent className='min-w-96 max-w-fit'>
				...
				<CredenzaBody className='space-y-4 pb-4 md:pb-0'>
					<div className='grid gap-2'>
						<Label htmlFor='size'>{t('modal-show-product-label.size')}</Label>
						<Select
							value={size}
									...
									<SelectItem
										...
										<div className='flex gap-1 items-center'>
											<span className='capitalize'>
												{t('modal-show-product-label.size', { context: size })}
											</span>
											<span className='text-muted-foreground'>
												-{' '}
												{t('modal-show-product-label.size-desc', {
													context: size,
												})}
											</span>
										</div>
					...
					<div className='border rounded-md'>
						{size == 'small' ? (
							product.text1.length > 25 || product.text2.length > 25 ? (
								<div
									ref={ref}
									className={cn('print:w-[51mm] print:h-[21mm] w-96')}>
									<div className='p-1.5 space-y-1.5'>
										<div className='print:text-xs line-clamp-2 leading-tight font-semibold'>
											{product.text1}
										</div>
										<div className='flex justify-between'>
											<div className='flex flex-col gap-1 justify-between'>
												<div className='print:text-[9px] text-sm font-medium line-clamp-2 leading-tight'>
													{product.text2}
												</div>
											...
											<div className='flex flex-col items-center justify-between'>
												...
												/>
												<span className='print:text-[9px] text-sm font-semibold w-max justify-self-end'>
													{product.barcode}
												</span>
											</div>
										</div>
								...
								<div ref={ref} className={cn('print:w-[51mm] print:h-[21mm]')}>
							...
							product.text1.length > 25 || product.text2.length > 25 ? (
								<div
									...
									<div className='p-2.5 flex flex-col space-y-2 h-full items-stretch'>
										<div className='print:text-xl text-base font-semibold'>
											{product.text1}
										</div>
										<div className='flex justify-between h-full'>
											<div className='flex flex-col justify-between'>
												<div className='print:text-sm text-sm font-medium line-clamp-2'>
													{product.text2}
												</div>
											...
											<div className='flex flex-col items-center justify-end'>
												...
												/>
												<span className='print:text-xs text-xs font-medium w-max justify-self-end'>
													{product.barcode}
												</span>
											</div>
										</div>
							...
							<div
								ref={ref}
								className={cn('w-[15cm] h-[8cm]', 'print:w-auto print:h-auto')}>
								<div className='p-6 flex flex-col space-y-4 h-full items-stretch'>
									<div className='print:text-5xl text-3xl font-semibold'>
										{product.text1}
									</div>
									<div className='flex justify-between h-full'>
										<div className='flex flex-col justify-between'>
											<div className='print:text-3xl text-2xl font-medium line-clamp-2'>
												{product.text2}
											</div>
