...
>(({ className, partial = false, isLoading = false, ...props }, ref) => (
		...
		<SwitchPrimitives.Thumb
			...
			{isLoading && (
				<div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center'>
					<Icons.spinner className='animate-spin size-3 text-primary' />
				</div>
			)}
			{partial && (
				<span className='absolute top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%] block bg-primary h-[2px] w-2' />
			)}
		</SwitchPrimitives.Thumb>
	</SwitchPrimitives.Root>
))
