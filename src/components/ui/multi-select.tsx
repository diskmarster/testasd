...
export function MultiSelect({
  ...
  const handleUnselect = (value: string) => {
    const option = options.find((option) => option.value === value)
		if (!option) return
    onChange(selected.filter((item) => item !== value), option)
  }
  ...
  const displaySelected =
    maxDisplayItems && selected.length > maxDisplayItems ? selected.slice(0, maxDisplayItems) : selected
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className={cn("min-h-9 h-auto w-full justify-between px-3", className)}
              ...
              {options.map((option) => {
                ...
                return (
                  <CommandItem
                    ...
                    className={cn("flex items-center gap-2 capitalize", option.disabled && "cursor-not-allowed opacity-60")}
                  >
                    <div
                      className={cn(
