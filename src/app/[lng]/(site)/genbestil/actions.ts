...
export const bulkAddOrderedToReorderAction = editableAction
  ...
  .action(async ({ parsedInput, ctx }) => {
    ...
    const promises = []
		...
		promises.push(ordersService.create(newOrder.meta, newOrder.lines))
    ...
    const arr = XLSX.write(workbook, { type: 'array' })
		...
		if (fileInfo.success) {
			promises.push(fileService.upload({ key: fileInfo.key, mimeType: fileInfo.type, body: arr }))
			promises.push(attachmentService.create({
      customerID: ctx.user.customerID,
      refDomain: 'genbestil',
      refID: id,
    ...
    await Promise.all(promises)
		...
		redirect(`/${ctx.lang}/genbestil/${id}`)
  })
