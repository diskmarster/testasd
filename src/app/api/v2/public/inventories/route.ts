...
export async function POST(
	...
	if (isMaintenanceMode()) {
		return apiResponse.locked(t('maintenance'), getVercelRequestID(headers()))
	}
	...
	if (!regulate.success) {
		...
		return apiResponse.internal(msg, getVercelRequestID(headers()))
	}
	...
	if (!regulate.data.ok) {
		...
		switch (status) {
			case 400:
				return apiResponse.badRequest(msg, getVercelRequestID(headers()))
