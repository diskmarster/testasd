...
export async function GET(request: NextRequest) {
  ...
  if (!secret) {
    return sendResponse(401, { msg:'authorization is missing' , error: 'authorization is missing' })
  }

  const parts = secret.split(' ', 2)
  if (parts[0] != 'Bearer') {
    return sendResponse(401, { msg: 'authorization is malformed', error: 'authorization is malformed' })
  }

  if (parts[1] != CRON_SECRET) {
    return sendResponse(401, { msg: 'authorization is denied', error: 'authorization is denied' })
  }
  ...
  if (!isKeyofCustomerMailSetting(mailType)) {
    return sendResponse(400, { msg: 'invalid mail type query parameter', error: 'invalid mail type query parameter' })
  }

  const mailRes = await tryCatch(customerService.getMailsForCron(mailType))
  if (!mailRes.success) {
    const errMsg =
      mailRes.error instanceof Error ? mailRes.error.message : 'unknown error occured'
...
): key is keyof Pick<CustomerMailSetting, 'sendStockMail' | 'sendMovementsMail'> | undefined {
  const validMailTypes: (keyof CustomerMailSetting)[] = ['sendStockMail', 'sendMovementsMail']
