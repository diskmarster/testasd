...
export const customer = {
	...
	getAllMailSettings: async function (
		...
		return await tx
			...
			.orderBy(
				customerMailSettingsTable.locationID,
				desc(customerMailSettingsTable.id),
			)
			.innerJoin(
				locationTable,
				eq(locationTable.id, customerMailSettingsTable.locationID),
			)
	...
	getMailSettingsForCron: async function (
		...
		return await tx
			...
			.from(customerMailSettingsTable)
			.where(typeCol ? eq(typeCol, true) : undefined)
			.orderBy(
				customerMailSettingsTable.locationID,
				desc(customerMailSettingsTable.id),
			)
	...
	deleteApiKey: async function (
		...
		const res = await tx
			.delete(apikeysTable)
			.where(
				and(eq(apikeysTable.key, key), eq(apikeysTable.customerID, customerID)),
			)
		return res.rowsAffected == 1
	},
